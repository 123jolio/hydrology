...
# Sidebar Parameters
with st.sidebar:
    st.header("Terrain Parameters")
    scale = st.slider("Elevation Scale", 0.1, 5.0, 1.0, help="Scale factor for STL elevation.")
    offset = st.slider("Elevation Offset (m)", -100.0, 100.0, 0.0, help="Vertical adjustment for DEM.")
    grid_res = st.number_input("Grid Resolution", 100, 1000, 500, help="Number of grid cells per axis.")
    
    st.header("Hydrology Parameters")
    rainfall = st.number_input("Rainfall (mm/hr)", 0.0, 100.0, 20.0, help="Rainfall intensity.")
    duration = st.number_input("Rainfall Duration (hr)", 0.1, 24.0, 2.0, help="Duration of rainfall event.")
    area_ha = st.number_input("Catchment Area (ha)", 0.1, 1000.0, 10.0, help="Catchment area in hectares.")
    runoff_coeff = st.slider("Runoff Coefficient", 0.0, 1.0, 0.3, help="Fraction of rainfall becoming runoff.")
    recession = st.number_input("Recession Rate (1/hr)", 0.1, 2.0, 0.5, help="Rate of flow decrease post-event.")
    
    st.header("Risk Parameters")
    risk_slope_w = st.slider("Slope Weight", 0.0, 2.0, 1.0, help="Weight for slope in risk calculation.")
    risk_dem_w = st.slider("DEM Weight", 0.0, 2.0, 1.0, help="Weight for elevation in risk calculation.")
