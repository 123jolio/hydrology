...
with tabs[0]:
    ...
    with st.expander("Burned Area Effects", expanded=True):
        st.markdown("**Runoff Increase Factor**: Multiplier for runoff in burned areas.")
        burn_factor = st.slider("Runoff Increase Factor", 0.0, 2.0, 1.0, 0.1, key="burn_factor")
        
        st.markdown("**Burned Area Threshold**: Pixel threshold (0–255) for burned area detection.")
        burn_threshold = st.slider("Burned Area Threshold", 0, 255, 200, 1, key="burn_threshold")
        
        st.markdown("**Band for Burned Area Threshold**: Choose the color band for thresholding.")
...
if 'processed_data' in st.session_state and st.session_state.processed_data is not None:
    ...
    st.markdown("**Instructions**: Adjust elevation and flow parameters to see how they impact the DEM visualization and hydrograph.")

    with st.expander("Visualization Options", expanded=True):
        st.markdown("**Show Burned Areas Overlay**: Toggle to overlay burned areas (red) on the DEM for context.")
        if 'show_burned_dem' not in st.session_state:
            st.session_state.show_burned_dem = False
        show_burned = st.checkbox("Show Burned Areas Overlay", value=st.session_state.show_burned_dem, key="dem_burned")
        st.markdown("**Burned Areas Transparency**: Adjust transparency of the burned overlay.")
        if 'burn_alpha_dem' not in st.session_state:
            st.session_state.burn_alpha_dem = 0.5
        burn_alpha = st.slider("Burned Areas Transparency", 0.0, 1.0, st.session_state.burn_alpha_dem, 0.1, key="dem_alpha")
    ...
    st.markdown("**DEM & Flow Visualization**: Shows terrain elevation (m) with flow direction arrows (blue).")
    ...
    with tabs[1]:
        ...
        st.markdown("### Distribution of burned areas from the uploaded TIFF.")
        if st.session_state.processed_data['burned_mask'] is not None:
            fig, ax = plt.subplots()
            cmap_display = ListedColormap(['red', 'black'])
            im = ax.imshow(
                np.flipud(burned_mask), cmap=cmap_display, origin='upper',
    ...
    with tabs[4]:
        ...
        if st.session_state.processed_data['retention_time'] is not None:
            st.write(f"Estimated Retention Time: {retention_time:.2f} hr")
            st.markdown("**Retention Time**: Indicates how long water is held before draining.")
        else:
            st.write("No effective runoff → Retention time not applicable.")

    with tabs[5]:
        st.header("GeoTIFF Export")
        st.markdown("### Export functionality is not yet implemented.")
    ...
    with tabs[10]:
        st.header("Scenario GIFs")
        st.markdown("### Animated GIF generation is not yet implemented.")

    with tabs[11]:
        st.header("Burned-Area Hydro Impacts")
        st.markdown("""
        **How Burned Areas Affect Hydrogeology**  
        - **Reduced Infiltration** → More surface runoff  
        - **Accelerated Erosion** → Higher sediment loads  
        - **Decreased Groundwater Recharge**  
        - **Nutrient & Ash Loading** → Potential water quality issues  
        """)
        ...
        if st.session_state.processed_data['burned_mask'] is not None:
            ...
            **Interpretation**:  
            ...
            st.warning("No burned area detected or TIFF missing. Upload a valid burned-area TIFF to see advanced impacts.")

    with tabs[12]:
        st.header("Parameter Comparison")
        st.markdown("### Comparison of hydrological parameters between burned and unburned areas.")
        if st.session_state.processed_data['burned_mask'] is not None:
            grid_z = st.session_state.processed_data['grid_z']
            slope = st.session_state.processed_data['slope']
            aspect = st.session_state.processed_data['aspect']
            ...
            if comparison_data:
                ...
                st.markdown("**Parameter Comparison Table**: Shows means, medians, and standard deviations for burned vs. unburned areas.")
                st.write(df)
            else:
...
# -----------------------------------------------------------------------------
# 11. Helper function for plotting with burned area overlay
# -----------------------------------------------------------------------------
